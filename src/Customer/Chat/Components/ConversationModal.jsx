...
import { Modal, View, Text, TextInput, Button, FlatList, StyleSheet, TouchableOpacity, Image, Platform, Dimensions } from 'react-native';
    ...
    if (item?.content === 'Attachment') {
      return (
        ...
        <View
          ...
          <Image source={{ uri: item?.media_upload?.url }} style={styles.messageImage} />
        </View>
      );
    }
    ...
    if(item?.content)
      return (
        <View style={[
            ...
            ((item?.sender_id === authId) ? styles.sentMessage : styles.receivedMessage)
          ]}
        >
          {
            item?.sender?.profile_image &&
              <Image
                source={{
                  uri: item?.sender?.profile_image ? Routes.serverUrl + '/storage/uploads/' + item?.sender?.profile_image : `${Routes.serverUrl}assets/images/ph-avatar.jpg`
                }}
                style={styles.image}
          ...
          <Text style={[styles.messageText, ((item?.sender_id === authId) ? { color: '#FFFFFF' } : {})]}>{item.content}</Text>
        </View>
      )
  };
    ...
    <Modal visible={isVisible} onRequestClose={onClose} animationType="slide">
      <View style={styles.modalContainer}>
        ...
        <View style={styles.header}>
          ...
          <Text style={styles.headerText}> { user ? user?.name : 'Anonymous Person' } </Text>
