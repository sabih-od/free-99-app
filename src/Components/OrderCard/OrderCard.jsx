...
import { faDollarSign, faFileInvoiceDollar, faStar } from '@fortawesome/free-solid-svg-icons';
import {
    generalFontSize,
    GlobalStyle,
    itemBg,
    ...
    return (
        <>
            <View style={[styles.orderCard, addReview && { borderRadius: 0, borderTopLeftRadius: 10, borderTopRightRadius: 10 }]}>
                <Image
                    style={styles.image}
                    source={{ uri: item?.product?.image ?? 'https://t4.ftcdn.net/jpg/02/51/95/53/360_F_251955356_FAQH0U1y1TZw3ZcdPGybwUkH90a3VAhb.jpg', cache: 'force-cache' }}
                />
                <View style={{ flex: 1 }}>
                    <Text style={styles.heading}>{product.name}</Text>
                    {isReview && (
                        <View style={[GlobalStyle.row, GlobalStyle.aic, gap(2), { position: 'absolute', right: 0, top: 5 }]}>
                    ...
                    <View style={[GlobalStyle.row, { gap: 20, justifyContent: 'flex-start', flex: 1, flexWrap: 'wrap' }]}>
                        <View>
            ...
            {addReview && (
                <>
                    ...
                    <AddReview modalIsVisible={modalVisible} closeModal={toggleReviewModal} itemId={product?.id} />
                </>
            )}
        </>
    );
    ...
    return (
        ...
        <View style={GlobalStyle.orderBox}>
            <View style={GlobalStyle.orderContent}>
                ...
                <Text style={GlobalStyle.orderMinText}>{moment(item.created_at).format('DD-MM-YYYY | LT')}</Text>
            ...
            <View style={{ alignItems: 'flex-end', justifyContent: 'space-between', flex: 1, gap: 10 }}>
                <View style={[GlobalStyle.row, GlobalStyle.ac]}>
                    <Text style={GlobalStyle.orderPrice}>
                        <FontAwesomeIcon icon={faDollarSign} color={textColor} size={generalFontSize - 4} />
                        {Math.floor(item?.total_price)}
                    </Text>
                    {/* <Text style={GlobalStyle.orderPriceStatus}>{item.payment_method}</Text> */}
                </View>
                <TouchableOpacity onPress={() => navigation.navigate('vendorOrderDetails', { item })} style={[GlobalStyle.themeBtn, GlobalStyle.invoiceBtn]}>
                    <FontAwesomeIcon icon={faFileInvoiceDollar} color={whiteColor} size={60} />
                </TouchableOpacity>
            </View>
        </View>
