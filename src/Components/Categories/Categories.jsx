...
const Categories = ({
  ...
  return (
    ...
    //       columnWrapperStyle={
    //         isVertical ? {gap: 15, justifyContent: 'space-between'} : null
    //       }
    ...
    //                   ? {uri: item?.image}
    //                   : {
    //                       uri: 'https://t4.ftcdn.net/jpg/02/51/95/53/360_F_251955356_FAQH0U1y1TZw3ZcdPGybwUkH90a3VAhb.jpg',
    //                       cache: 'force-cache',
    //                     }
    //               } // Constructing image source path
    //             />
    ...
    <View>
      ...
      {category ? (
        <ScrollView
          ...
          contentContainerStyle={{flexDirection: 'row', gap: 20}} // Adjust gap between items
        >
          {category.map((item, index) => (
            <TouchableOpacity
              key={item.id.toString()} // Use item's unique id as key
              onPress={() => {
                onCategoryPress();
                activeCategory(item.slug); // Logging current route name on item press
              }}
