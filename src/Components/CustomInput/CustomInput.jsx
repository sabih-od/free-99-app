...
import { whiteColor, fontFamily, generalFontSize, themeColor } from '../../Styles/Theme';

const CustomInput = ({ placeholderText, isPassword, isEmail, isNumber, onChangeText, value }) => {
    ...
    const keyboardType = isNumber ? 'phone-pad' : isEmail ? 'email-address' : 'default';

    return (
        <View style={[styles.inputContainer, isFocus && styles.activeBorder]}>
            {value === '' && <Text style={styles.placeholder}>{placeholderText}</Text>}
            <TextInput
                style={styles.input}
            ...
            {isPassword && (
                <TouchableOpacity
                    ...
                    style={[styles.showPassword, styles.icon]}
                >
